...
datimiei<-as.data.frame(read_excel("../datasets/dataset_completo.xlsx",sheet = 4))
...
esito_test<-function(costante,campione,alfa){
  trasformazione_dati<-trunc(costante*log(campione+1))
  dati<-trasformazione_dati-min(trasformazione_dati)
  massimo<-max(dati)
...
j<-0.000
for (i in 1:10000){
  esiti[i]<-esito_test((c+j),datimiei$`num rischi occorsi per settimana`,0.10)
  cc[i]<-c+j
  j<-j+0.001
}
#cbind(cc,esiti)
...
{chisquares[i]<-trovamiglior_c(valc1[i],datimiei$`num rischi occorsi per settimana`,0.10)}
...
j<-0.000
for (i in 1:10000){
  esiti[i]<-esito_test((c+j),datimiei$`num rischi occorsi per settimana`,0.05)
  cc[i]<-c+j
  j<-j+0.001
}
#cbind(cc,esiti)
...
{chisquares[i]<-trovamiglior_c(valc2[i],datimiei$`num rischi occorsi per settimana`,0.05)}
round(chisquares,digits=4)
...
j<-0.000
for (i in 1:10000){
  esiti[i]<-esito_test((c+j),datimiei$`num rischi occorsi per settimana`,0.01)
  cc[i]<-c+j
  j<-j+0.001
}
#cbind(cc,esiti)
...
{chisquares[i]<-trovamiglior_c(valc3[i],datimiei$`num rischi occorsi per settimana`,0.01)}
round(chisquares,digits=4)
