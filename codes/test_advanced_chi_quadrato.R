...
esito_test<-function(costante,campione,alfa,gradilib){
dati<-trunc(costante*log(campione+1))
massimo<-max(dati)
freqosservate<-table(cut(dati,breaks = c(0,1,2,3,4,5,massimo+0.01),right = FALSE))
lambda<-mean(dati)
...
if(sum(a)<6){return(0)}#ci devono essere almeno 5 osservazioni in ogni classe
if(sum(b)<6){return(0)}#tutte le frequenze attese delle 6 classi devono essere >=5
...
trovamiglior_c<-function(costante,campione,alfa,gradilib){
dati<-trunc(costante*log(campione+1))
massimo<-max(dati)
freqosservate<-table(cut(dati,breaks = c(0,1,2,3,4,5,massimo+0.01),right = FALSE))
lambda<-mean(dati)
...
j<-0.000
for (i in 1:1000){
  esiti[i]<-esito_test((c+j),datimiei$numero_acquisti,0.10,4)
  cc[i]<-c+j
  j<-j+0.001
}
#cbind(cc,esiti)
...
for (i in 1:length(valc1))
  {chisquares[i]<-trovamiglior_c(valc1[i],datimiei$numero_acquisti,0.10,4)}
chisquares
...
j<-0.000
for (i in 1:1000){
  esiti[i]<-esito_test((c+j),datimiei$numero_acquisti,0.05,4)
  cc[i]<-c+j
  j<-j+0.001
}
#cbind(cc,esiti)
...
{chisquares[i]<-trovamiglior_c(valc2[i],datimiei$numero_acquisti,0.05,4)}
...
j<-0.000
for (i in 1:1000){
  esiti[i]<-esito_test((c+j),datimiei$numero_acquisti,0.01,4)
  cc[i]<-c+j
  j<-j+0.001
}
#cbind(cc,esiti)
...
{chisquares[i]<-trovamiglior_c(valc3[i],datimiei$numero_acquisti,0.01,4)}
chisquares
...
#GRAFIci singoli
#plot(valc1,y1,xlim = c(0.940,0.952),ylim = c(0.000,0.200),col="red")
#plot(valc2,y2,xlim = c(0.932,0.962),ylim = c(0.000,0.200),col="green")
#plot(valc3,y3,xlim = c(0.910,1.008),ylim = c(0.000,0.200),col="blue")

#GRAFICO ACCORPATO 1
plot(grafico1$ascisse1,grafico1$ordinate1,xlim = c(0.910,1.008),ylim = c(0.000,0.200),xlab = "valori di c",ylab = "valori di alfa")
axis(1,at=seq(0.910,1.100,by=0.01),las=0)
...
#GRAFICO ACCORPATO 2 (con ascisse e ordinate invertite)
plot(grafico2$ascisse2,grafico2$ordinate2,xlim = c(0.000,0.200),ylim = c(0.910,1.008),xlab = "valori di alfa",ylab = "valori di c")
